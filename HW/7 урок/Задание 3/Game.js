...
class Game {
     ...
     * 1. Показывает длину змейки.
     * 2. перемещение змейки.
    ...
    /**
     * Метод проставляет счет игры (длину змейки).
     */
    showCount() {
        let text = this.snake.body.length;
        this.countEl.insertAdjacentHTML('beforeend', text);
    }

    /**
     * Метод проверяет выиграна ли игра, останавливает игру, выводит сообщение о выигрыше.
     * @returns {boolean} - если длина змейки достигла длины, нужной для выигрыша, тогда true,
     * иначе false.
     */
    isGameWon() {
    ...
    /**
     * Метод проверяет проиграна ли игра, останавливает игру, выводит сообщение о проигрыше.
     * @returns {boolean} - если мы шагнули в стену, тогда true, иначе false.
     */
    isGameLost() {
        if (this.board.isNextStepToSnakeBody(this.snake.body[0])) {
            clearInterval(this.tickIdentifier);
    ...
    /**
     * В зависимости от нажатой кнопки (вверх, вниз, влево, вправо) будет вызываться
